...
import com.luckyone.web.model.entity.Command.CommandType;
        ...
        try {
            ...
            switch (CommandType.match(command.getCode())) {
                case CONNETION: // 如果是连接消息就不需要做任何操作了，因为连接上的话在token鉴权那就做了
                    ...
                    break;
                case CHAT_SEND:
                    ChatHandler.send(channelHandlerContext,command);
                    break;
                case CHAT_WITHDRAW:
                    ChatHandler.withdraw(channelHandlerContext, command);
                    break;
                default: channelHandlerContext.channel().writeAndFlush(IMResponse.error("不支持的CODE " + command.getCode()));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
